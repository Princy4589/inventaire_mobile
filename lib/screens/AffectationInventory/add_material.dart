...
import 'package:pgi_mobile/screens/User/list_user_staff.dart';
...
class StaffExtension extends StatelessWidget {
  const StaffExtension({Key? key}) : super(key: key);
...
class _StaffWidget extends State<StaffWidget> {
  ...
  final List<Map<String, dynamic>> _managements =
      []; //pour stocker la liste des directions
  final List<Map<String, dynamic>> _headquarters =
      []; //pour stocker la liste des sièges
  final List<Map<String, dynamic>> _services =
      []; //pour stocker la liste des services
                              ...
                              onPressed: () async {
                                if (_formKey.currentState!.validate()) {
                                  if (formSubmit['Managements'] == null ||
                                    ...
                                    //Enregistre les choix utilisateurs dans le "local storage"
                                    await saveFormSubmit(formSubmit);
                                    Navigator.pushReplacement(
                                        context,
                                        MaterialPageRoute(
                                            builder: (context) =>
                                                const ListUserAffectedExtension()));
                                  }
                                }
                              },
